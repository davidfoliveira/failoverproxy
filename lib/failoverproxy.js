...
function _buildInternals() {
	...
	if ( opts.local ) {
		if ( opts.local.address )
			self.localAddress = opts.local.address;
		if ( opts.local.port && opts.local.port.toString().match(/^\d+$/) )
			self.localPort = opts.local.port;
	}
	...
	opts.backends.forEach(function(be){
		...
		if ( typeof be == "string" ) {
			if ( be === "cache" ) {
				...
				if ( !opts.cache.keyGenerator )
					opts.cache.keyGenerator = function(req){return req.url};
	...
	// HTTP sockets
	if ( opts.httpMaxSockets && typeof opts.httpMaxSockets == "number" )
		self.httpMaxSockets = opts.httpMaxSockets;
	http.globalAgent.maxSockets = self.httpMaxSockets;
...
//	req.headers['host'] = be.host;
	...
	if ( typeof(req.POSTContent) == "undefined" ) {
		req.on('data',function(chunk) {
			...
			req._POSTContent = (req._POSTContent == null) ? Buffer.concat([chunk]) : Buffer.concat([req._POSTContent,chunk]);
		});
		req.on('end', function() {
